// frontend/src/store/useStatusStore.js
...
socket.on("status_viewed", ({ statusId, viewer }) => {
  set((state) => ({
    statuses: state.statuses.map((status) => {
      if (String(status._id) !== String(statusId)) return status;

      const alreadyViewed = status.viewers?.some(
        (v) => String(v._id) === String(viewer._id)
      );
...
createStatus: async (statusData) => {
  ...
  try {
    ...
    // add to statuses in local state
if (data.data) {
  set((state) => ({
    statuses: state.statuses.some(
...
toggleLikeStatus: async (statusId) => {
  try {
    const { data } = await axiosInstance.put(
      `/status/${statusId}/like`
    );

    // 🔁 update local status likes
    set((state) => ({
      statuses: state.statuses.map((status) =>
        status._id === statusId
          ? { ...status, likes: data.data }
          : status
      ),
    }));
  } catch (error) {
